...
Nettoyage des fichiers CSV de transactions mobile money et génération des
deux tableaux croisés (par bénéficiaire / par expéditeur). Reprend
exactement la logique déjà validée (Excel/Python puis JavaScript) :
- délimiteur virgule
...
def _write_pivot_sheet(wb, sheet_name, title, name_col, df):
    ...
    for name in names:
        ...
        for d in dates:
            ...
            total_sum += s
            total_cnt += c
        rows.append({"name": name, "per_date": per_date, "total_sum": total_sum, "total_cnt": total_cnt})
    ...
    for d in dates:
        ...
        sc, cc = ws.cell(row=gt_row, column=col, value=s), ws.cell(row=gt_row, column=col + 1, value=c)
        sc.number_format = "#,##0"
        col += 2
...
def build_transactions_workbook(df: pd.DataFrame, source_label: str = "") -> Workbook:
    ...
    _write_pivot_sheet(wb, "TCD - To Name", "Tableau croisé — Par bénéficiaire (To Name)", "To name", df)
    _write_pivot_sheet(wb, "TCD - From Name", "Tableau croisé — Par expéditeur (From Name)", "From name", df)
    return wb


# ---------------------------------------------------------------------------
# Auto-test : python3 core/transactions.py <chemin_vers_fichier.csv>
# ---------------------------------------------------------------------------
if __name__ == "__main__":
    ...
    if not chemin_test:
        print("Usage : python3 core/transactions.py <chemin_vers_fichier.csv>")
        sys.exit(1)
